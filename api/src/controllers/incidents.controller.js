...
async function list(req, res, next) {
  try {
    ...
    if (status === 'open') where.status = 'OPEN';
      ...
      return res.status(400).json({ error: { code: 'BAD_STATUS', message: 'status must be open|closed|all' } });
    }
    if (req.query.sectorId) {
      if (!SECTORS.includes(req.query.sectorId)) {
        return res.status(400).json({ error: { code: 'BAD_SECTOR', message: 'sectorId must be A|B|C' } });
      }
